namespace core {
    export class Router {
         ...
         * @returns {void}
         */
        Add(routee: string): void {
            this.m_routingTable.push(routee)
        }
         ...
         * @returns {number} 
         */
        Find(routee: string): number {
            return this.m_routingTable.indexOf(routee)
        }
         ...
         * @returns {boolean}
         */
        Remove(routee: string): boolean {
            // if route is found
            if (this.Find(routee) > -1) {
                // remove the route
                this.m_routingTable.splice(this.Find(routee), 1)
                return true
            }
            return false
        }
...
let routee: string = location.pathname // alias for location.pathname
